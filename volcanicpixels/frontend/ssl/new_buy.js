...
$(document).ready(function(){
    ...
    var generateKeyPair = function() {
        ...
        var $this = $(this);
        $this.parents('fieldset').addClass('loading');
        $this.addClass('disabled');
        $this.parents('fieldset').find('.message').removeClass('hide');

        var reset = function() {
            $this.parents('fieldset').removeClass('loading');
            $this.parents('fieldset').find('.message').addClass('hide');
        }

        var _generateKeyPair = function() {

            try {
                keypair = forge.rsa.generateKeyPair(2048);
                reset();
                ...
                var text = forge.pki.privateKeyToPem(keypair.privateKey);
                $('.download').find('a').attr('href', 'data:application/octet-stream;charset=utf-8,' + encodeURIComponent(text));
                ...
                // something went horribly wrong. Apologise to the user and log this
                alert('Something went wrong. Reload the page and try again, if that doesn\'t work then try another browser?');
                reset();
        ...
        setTimeout(_generateKeyPair, 20);
    }

    $('.generate-keypair').click(generateKeyPair);

    $('.download a').click(function(){
        $('.have-downloaded').removeClass('regular-button').addClass('primary-button');
    });

    $('.have-downloaded').click(function(){
        $('.payment').removeClass('disabled');
        $('.have-downloaded').addClass('disabled');
    });
    ...
    var purchase = function() {
        ...
        var submitOrder = function() {
            // make ajax request
            checkEmailRequest = $.post('purchase_certificate', $('#form').serialize(), function(response){
        ...
        if (domain == undefined || domain.length == 0) {
            errorFunc("You haven't entered a domain");
            $('.domain').focus();
            return;
        }

        if (organization == undefined || organization.length == 0) {
            errorFunc("You haven't entered a business/individual name.");
            $('.organization').focus();
            return;
        }

        if (city == undefined || city.length == 0) {
            errorFunc("You haven't entered a city.");
            $('.city').focus();
            return;
        }

        if (state == undefined || state.length == 0) {
            errorFunc("You haven't entered a state.");
            $('.state').focus();
            return;
        }

        if (state.length < 3) {
            errorFunc("Enter the full state name.");
            $('.state').focus();
            return;
        }
        ...
        csrText = forge.pki.certificationRequestToPem(csr);
        $('.csr').val(csrText);
